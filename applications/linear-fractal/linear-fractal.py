...
class LinearFractal:
    ...
    def dump_model(self, path):
        ...
        self.quantile.dump(path + "/quantile")
    ...
    def load_model(self, path):
        if not self.table.load(path + "/weights"):
            return False
        elif not self.quantile.load(path + "/quantile"):
            return False
        else:
            return True
    ...
    def valid(self):
        if self.pas == self.conf.validate_model_interval:
            block = self.valid_data.Read("async", self.conf.mini_batch_size)
            label, sparse = self.parser(block)
